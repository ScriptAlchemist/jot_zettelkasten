...
If you want to change some of all items of the array, you can use `map()` to create a new array. The function you will pass to `map` can decide what to do with each item, based on its data or its index (or both).
...
Sometimes, you may want to insert an item at a particular position that's neither at the beginning nor the end. To do this, you can use the `...` array spread syntax together with the `slice()` method. The `slice()` method lets you cut a "slice" of the array. To insert an item, you will create an array that spread the slice before the insertion point, then the new item, and then the rest of the original array.
...
export default function List() {
  ...
  function handleClick() {
    const insertAt = 1; // Count be any index
    const nextArtist = [
...
Objects are not really located "inside" array. They might appear to be "inside" in code, but each object in an array is a separate value, to which the array "points". This is why you need to be careful when changing nested fields like `list[0]`. Another person's artwork list may point to the same element of the array!

When updating nested state, you need to create copies for the point where you want to update, and all the way up to the top level. Let's see how this works.

In this example, two separate artwork lists have the same initial state. They are supposed to be isolated, but because of a mutation, their state is accidentally shared, and checked a box in one list affects the other list:
...
export default function BucketList() {
  ...
  function handleToggleYourList(artwork, nextSeen) {
...
* If you don't want to change your state structure, you might prefer to use `Immer`, which lets you write using the convenient bu mutating syntax and takes car of producing the copies for you.
...
export default function BucketList() {
  ...
  return (
    <>
      ...
      <h2>Your likst of art to see:</h2>
      <ItemList
...
Behind the scenes, Immer always constructs the next state from scratch according to the changes that you've done to the `draft`. This keeps your event handlers very concise without every mutating state.
...
* You can use `filter()` and `map()` to create new array with filtered or transformed items
* You can use Immer to keep you code concise.
...
This shopping cart has working "+" button, but the "-" button doesn't do anything. You need to add an event handler to it so that pressing it decreases the `count` of the corresponding product. If you press "-" when the count is 1, the product should automatically get removed form the car. Make sure it never shows 0.
# Solutions
...
This is the same example as in the previous challenge. This time, fix the mutations by using Immer. For your convenience, `useImmer` is already imported, so you need to change to `todos` state variable to use it.
